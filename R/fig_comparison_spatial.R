...
fig_comparison_spatial <- function() {
  ...
  # Param
  maxVal <- c(max(disconnect[[1]], na.rm = TRUE),
              max(connect[[1]], na.rm = TRUE),
              max(anomalies[[1]], na.rm = TRUE))

  minVal <- c(min(disconnect[[1]], na.rm = TRUE),
  ...
  legendEGSL(range = c(0, maxVal[2]), pal = palImpact, cexMain = .9, cexSub = .65, n = 5,
  ...
  plot(st_geometry(egslSimple), col = 'transparent', border = focus, lwd = .75, add = T)
  legendEGSL(range = c(0, maxVal[1]), pal = palImpact, cexMain = .9, cexSub = .65, n = 5,
             mainTitle = 'Species-scale cumulative effects')

  # Letter
  pos <- par("usr")
  text(x = pos[1] + (pos[2]-pos[1])*0.009, y = pos[4] - (pos[4]-pos[3])*0.015, labels = 'b', cex = 1.5, font = 2)
  ...
  plot(st_geometry(egslSimple), col = 'transparent', border = focus, lwd = .75, add = T)
  legendEGSL(range = c(0, maxVal[3]), pal = palDist, cexMain = .9, cexSub = .65, n = 5,
             mainTitle = 'Transgressive properties', subTitle = 'log(Network-scale / Species-scale)')

  # Letter
  pos <- par("usr")
  text(x = pos[1] + (pos[2]-pos[1])*0.009, y = pos[4] - (pos[4]-pos[3])*0.015, labels = 'c', cex = 1.5, font = 2)
