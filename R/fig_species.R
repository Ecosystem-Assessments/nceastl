...
fig_species_spatial <- function() {
  ...
  for(i in 1:length(sp)) {
    ...
    # Letter
    pos <- par("usr")
    text(x = pos[1] + (pos[2]-pos[1])*0.01, y = pos[4] - (pos[4]-pos[3])*0.015, labels = letters[i], cex = 1.2, font = 2)
  }
  ...
  for(i in 1:length(sp)) {
    ...
    plot(st_geometry(egslSimple), col = 'transparent', border = focus, lwd = .75, add = T)
    legendEGSL(range = c(0, maxVal), pal = palImpact, cexMain = .9, cexSub = .65, n = 5,
