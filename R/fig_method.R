...
fig_method <- function() {
  ...
  plotMath <- function(spid = NULL, drid = NULL, pathid = NULL) {
    ...
    spnm <- c("\\textit{$E_{bal}$}","$\\textit{E_{bel}$}",
    ...
    # Drivers
    drnm <- c("\\textit{$D_{tem}$}", "\\textit{$D_{nav}$}",
            "\\textit{$D_{pec}$}")
    xdr <- c(-.4,0,.4)
    ydr <- 1.4
    ...
    # Susceptibilities
    unm <- c("\\textit{$\\mu_{cop,tem}$}", "\\textit{$\\mu_{bal,nav}$}",
