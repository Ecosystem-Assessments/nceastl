...
fig_contribution_taxa <- function() {
  ...
  # =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=#
  # Reorder data
  cekm <- dplyr::left_join(cekm, spList, by = c("Taxa" = "shortname")) |>
    dplyr::arrange(gr1, gr2, desc(total))
  ...
  di <- direct[, drList$drivers] %>%
    ...
    as.data.frame() %>%
    dplyr::mutate(id = 1:n()) %>%
    dplyr::gather("drivers", "ymax", -id) %>%
    dplyr::arrange(id) %>%
    dplyr::mutate(ymin = c(0, ymax[1:(length(ymax) - 1)])) %>%
    dplyr::filter(drivers != "temp") %>%
    dplyr::mutate(
      ymax = ymax + yG, ymin = ymin + yG,
      xmax = id + xG, xmin = id - xG
    ) %>%
    dplyr::left_join(drList[, c("drivers", "cols")], by = "drivers")
