...
fig_ncea <- function() {
  ...
  # Cumulative impacts legend
   ...
   z <- data.frame(y1 = yinit-ygap/8-ygap/2-1000,
  ...
  z <- data.frame(y1 = rep(yinit-1.25*ygap, 5),
                  stringsAsFactors = F) %>%
       mutate(cex = seq(0.5,7, length.out = 5),
              rel = cex/7,
              cex2 = cumsum(cex),
